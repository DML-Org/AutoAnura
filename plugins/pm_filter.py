...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="⚡How To Download⚡", url='https://t.me/ClickAdd_MCF')
        ]
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if groupids is None:
            ...
            return await query.answer('Please Share & Support')
        buttons = []
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>ඔයා Type කරපු නම : {search}\n\nFilm සහ TV Series දෙකම දැන් මේ Bot තමා දෙන්නෙ. Film එකක් නම් ඉස්සර විදිහටමයි. TV Series ලබාදෙන්නෙ 📺📺 ඉමෝජි දෙක මැදින් සීරිස් එකේ නම තියෙන විදිහට.\n\nඋදා : 📺Wednesday📺\n\n🧐 Link එකෙන් ගිහින් ෆිල්ම් එක ගන්න හැටි දැනගන්න පල්ලෙහා තියෙන How To Download කියන එක Click කරන්න.</b>"    if imdb and imdb.get('poster'):
        try:
